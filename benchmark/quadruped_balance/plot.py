...
def render(theme, path):
    ...
    ax.set_xlim(0, None)
    ax.set_ylim(-16, -1.2)  # scratch starts at -92; annotated instead of scaled
    ax.text(
        600,
        -15.4,
        "SAC from scratch enters from −92",
        color=c["muted"],
        fontsize=8.5,
        va="bottom",
    )
    ...
    ax.text(
        0,
        1.14,
        "Whole-body MPC carrying an unmodeled payload",
    ...
    ax.text(
        0,
        1.045,
        "Go2 balance, trunk mass ×2 vs the MPC's model · mean over 5 "
...
if __name__ == "__main__":
    for theme in ("light", "dark"):
        render(theme, os.path.join(OUT, f"quadruped_balance_{theme}.png"))
